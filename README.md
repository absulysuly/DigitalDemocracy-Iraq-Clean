...
Election-specific surfaces are gated behind the `VITE_ENABLE_ELECTIONS` (or `NEXT_PUBLIC_ENABLE_ELECTIONS`) environment variables. They default to `false`, which hides election navigation and stubs election APIs. To preview election features locally or in staging, set `VITE_ENABLE_ELECTIONS=true` before running the app so the UI and APIs reappear.
